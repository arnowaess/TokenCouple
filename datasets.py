...
def ResizeMaxSize(I, max_size) : 
    h, w = I.size
    ratio = max(h / float(max_size), w / float(max_size)) 
    new_h, new_w = int(round(h / ratio)), int(round(w / ratio)) 

    Iresize = I.resize((new_h, new_w), resample=Image.LANCZOS)
    
    return Iresize, ratio

class ImageDataset:
    def __init__(self, image_path, patch_size, resize=False, isRot=False):
        ...
        with open(image_path, "rb") as f:
            ...
            img = img.convert("RGB")
            if self.resize:
                img = ResizePatch(img, self.patch_size)

        # Build a dataloader
        img_orig = transform(img)
        self.img_size = list(img_orig.shape[-1:-3:-1])

        if self.isRot:
            Crot = np.random.choice(self.angleCls, 1, replace=False)
            img_rot = transform(img.rotate(angle=self.angle[Crot], resample=Image.BILINEAR))
            self.dataloader = [[img_orig, image_path, img_rot, Crot]]
        else:
            self.dataloader = [[img_orig, image_path]]
...
class VocDataset(VOCDetection):
    def __init__(self, dataset_name, root, year, image_set, remove_hards, max_size=None):
        super(VocDataset, self).__init__(root, year, image_set)
        """
    ...
    def __getitem__(self, index):
        ...
        if self.max_size is not None:
            img, resize_ratio = ResizeMaxSize(img, self.max_size)
        else:
            resize_ratio = None
    ...
    def get_orig_size(self, im_name):
        """
        Load the image corresponding to the im_name
        """
        if "VOC" in self.dataset_name:
...
class Dataset:
    def __init__(self, dataset_name, dataset_set, remove_hards, isRot=False):
        """
        Build the dataloader
        """
